...
def step_4_refine_chunks(sections: List[Dict[str, Any]], max_len: int) -> List[Dict[str, Any]]:
    ...
    # 阶段3：字段兜底 → 适配Milvus向量库schema（parent_title/part等为必填/非空字段）
    # 兜底规则：无parent_title则用自身title，title也无则填空字符串；part缺失或None则填0
    valid_sections = []
    for sec in final_sections:
        if not isinstance(sec, dict):
            logger.warning(f"文档切分兜底：跳过非字典类型切片，类型={type(sec)}")
            continue

        # part字段兜底：不存在或值为None时，默认0
        if "part" not in sec or sec["part"] is None:
            sec["part"] = 0

        # parent_title兜底：不存在、值为None、或为空字符串时，尝试用title兜底
        if not sec.get("parent_title"):
            sec["parent_title"] = sec.get("title") or ""

        # title兜底：确保title不为None
        if not sec.get("title"):
            sec["title"] = ""

        # file_title兜底：确保不为None
        if not sec.get("file_title"):
            sec["file_title"] = ""

        valid_sections.append(sec)

    final_sections = valid_sections
    logger.debug(f"步骤4-3：字段兜底完成，所有Chunk均包含完整字段，有效切片数：{len(final_sections)}")
